...
export const paymentInvoice = (user, name, invoiceNum, amount, file) => {
    const formData = new FormData();
    formData.append("user", user);
    if (name) {
...
export const createInvoice = (price_amount, order_id, description) => {
    ...
    const raw = JSON.stringify({
        ...
        "ipn_callback_url": "https://virafunding.com/result/success",
        "success_url": "https://virafunding.com/result/success",
        "cancel_url": "https://virafunding.com/result/failed"
    });
...
export const getPaymentInfo = async (paymentId) => {
    try {
        ...
        throw error; // Optional: You can choose to rethrow the error if needed
    }
};
